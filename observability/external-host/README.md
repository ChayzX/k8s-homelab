...
Use `alloy-linux.alloy.example` for a Unix PC that cannot be reached from the
homelab LAN. Alloy scrapes the local host and sends a deliberately small,
...
3. Create the token file as root, containing only the Grafana Cloud
   `metrics:write` token:
...
5. In Grafana Cloud, query `up{job="host",site="remote"}` and
   `node_memory_MemAvailable_bytes{site="remote"}` to confirm receipt.
...
per-process, container, textfile, and other high-cardinality collectors to
protect the free-tier budget.
