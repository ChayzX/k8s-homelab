...
| Loki / Alloy | `observability/loki*.yaml` and `observability/alloy-logs*.yaml` | `observability` namespace | Loki stores logs; Alloy labels and parses logs. Changes to labels affect dashboards and the host watcher, so update queries/tests together. |
...
- `observability/loki-config.yaml` and `observability/alloy-logs-*.yaml`
...
- `pantry-bot/60-deployment-cloudflared.yaml`,
  ...
  Cloudflare; use `docs/recovery/PANTRYBOT-COMMANDS-TUNNEL-PLAN.md` for the
  staged commands hostname cutover.
