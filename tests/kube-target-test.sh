...
chmod +x "$ROOT/tests/fixtures/kubectl-target-ready" \
  "$ROOT/tests/fixtures/kubectl-target-oracle-ready" \
  "$ROOT/tests/fixtures/kubectl-target-wrong"
...
if KUBECTL_BIN="$ROOT/tests/fixtures/kubectl-target-oracle-ready" \
  "$ROOT/scripts/assert-kube-target.sh" oracle; then :; else
  echo "ready oracle target should pass" >&2; exit 1
fi

if KUBECTL_BIN="$ROOT/tests/fixtures/kubectl-target-ready" \
  "$ROOT/scripts/assert-kube-target.sh" oracle; then
  echo "home fixture must fail the oracle target check" >&2; exit 1
fi
