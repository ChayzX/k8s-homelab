...
grep -q 'target_label  = "host"' "$config"
grep -q 'target_label = "site"' "$config"
grep -q 'replacement  = "remote"' "$config"
...
grep -q 'target_label  = "host"' "$windows_config"
grep -q 'target_label = "site"' "$windows_config"
grep -q 'replacement  = "remote"' "$windows_config"
! grep -Eq 'glsa_|glc_|password = "[A-Za-z0-9_/-]{20,}"' "$windows_config"
! grep -Eq 'regex = ".*(^|\\|)(process_|service_|container_|go_)' "$windows_config"
