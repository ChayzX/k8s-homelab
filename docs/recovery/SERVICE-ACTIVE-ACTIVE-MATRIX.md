...
|---|---|---|---|---|---|---|
| PantryBot | Stateful, event-processing | PostgreSQL authority; one fenced writer per epoch | Twitch chat/EventSub, overlay, moderation; PostgreSQL lease/fencing per target | Split HTTP/UI, worker, overlay, gateway, dispatcher, and tunnel capacity Ready against Oracle writer; database fenced | Writable promoted database and matching active application capacity | Reference implementation. Both sites now have Ready split capacity; Oracle owns all external-side-effect leases in the current failover epoch. Return-home, synthetic event proof, and automatic promotion remain open. See PantryBot #147 and homelab #191. |
