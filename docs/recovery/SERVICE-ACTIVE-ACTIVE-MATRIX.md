...
|---|---|---|---|---|---|---|
| PantryBot | Stateful, event-processing | PostgreSQL authority; home writable, Oracle physical standby | Twitch chat/EventSub, overlay, moderation; PostgreSQL lease/fencing per target | Active application capacity; writable database | Active application capacity; read-only streaming standby | Reference implementation. Split roles, durable queue/outbox, public routing, cross-site application capacity, persistent return transport, and single-owner lease behavior are live. Remaining gates are automated promotion, Minecraft-safe source fencing, and full failure evidence. See PantryBot #147 and homelab #191. |
...
| Authentik + PostgreSQL | Stateful identity | PostgreSQL PVC/physical standby | Identity/session issuance; duplicate active writers are unsafe | Active primary capacity | Active server/worker/LDAP capacity against home writer; local physical standby | Oracle application capacity is now live against the home writer with matching DB credentials, while its local PostgreSQL stays read-only. Promotion still requires writer fencing, endpoint switch, provider/session validation, emergency access, and rollback evidence. See homelab #168/#198/#202. |
