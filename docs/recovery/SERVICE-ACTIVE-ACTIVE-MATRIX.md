...
| Operations dashboard | Stateful web/API | SQLite backup to R2; Authentik session dependency | Browser/API mutations and audit writes; no autonomous external writer | Active capacity, current writer | Active read/API capacity with a mutation ownership boundary until shared transactional state is deployed | Do not call a zero-replica recovery overlay active-active. Keep Oracle live and observable; migrate mutable audit/session state to a portable transactional authority before enabling mutations at both sites. See homelab #201. |
