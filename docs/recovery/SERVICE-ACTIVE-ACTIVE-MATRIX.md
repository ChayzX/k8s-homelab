...
|---|---|---|---|---|---|---|
| PantryBot | Stateful, event-processing | PostgreSQL target; SQLite retained only for rollback/migration | Twitch chat/EventSub, overlay, moderation; PostgreSQL lease/fencing per target | Active application capacity and current writer | UI/application capacity; Pantry PostgreSQL standby currently scaled to zero | Controlled promotion/failback and stale-owner fencing evidence exists, but continuous Oracle standby capacity is not currently active. Remaining gates are standby reactivation and freshness proof, production stale-writer rejection, provider-side external-effect semantics, public failover routing, and broader site/failure evidence. GHCR-authenticated component rollout is verified. See PantryBot #147 and homelab #191. |
