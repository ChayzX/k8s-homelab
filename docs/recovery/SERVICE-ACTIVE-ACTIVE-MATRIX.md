...
|---|---|---|---|---|---|---|
| PantryBot | Stateful, event-processing | PostgreSQL authority; one fenced writer per epoch | Twitch chat/EventSub, overlay, moderation; PostgreSQL lease/fencing per target | Fenced database; HTTP identity capacity only | Writable promoted database and active application capacity | Reference implementation. Split roles, durable queue/outbox, public routing, cross-site capacity, scoped GCP->ChaseBot source fencing, and Oracle lease ownership were exercised in the 2026-09-13 controlled failover. Return-home, synthetic event proof, and automatic promotion remain open. See PantryBot #147 and homelab #191. |
