...
| Opsbot | Stateful operational controller | Bot-local state plus Kubernetes API/RCON access | Discord messages, pod control, Minecraft/RCON commands; one owner per command stream | Active capacity, one side-effect owner | Active standby/recovery capacity, no duplicate owner | Keep one fenced Discord/mutation owner. Oracle may host a ready replica only after credential/API scope, lease loss, stale-owner rejection, and controlled return-home evidence. See homelab #200 and homelab #191. |
| Operations dashboard | Stateful web/API | SQLite backup to R2; Authentik session dependency | Browser/API mutations and audit writes; no autonomous external writer | Active capacity, single SQLite writer | Recovery overlay at replicas 0; restore-capable application capacity | Do not run active-active SQLite writers. Operations PR #157 provides the Oracle recovery overlay; still prove emergency access, restore freshness, local writable promotion, writer fencing, and routing. See homelab #201. |
| Authentik + PostgreSQL | Stateful identity with an active-active application tier | Shared Authentik state with one fenced PostgreSQL writer per database epoch | Identity/session issuance; both sites may serve requests, but database promotion and epoch fencing prevent stale writers | Active server/worker/LDAP capacity | Active server/worker/LDAP capacity using portable secrets and the current database authority | Run the Authentik application tier in both sites. Keep one database writer per epoch, promote only after old-writer fencing, and prove provider/secret/session parity, emergency access, non-production login, routing, and rollback. See homelab #198/#202. |
...
| CI deployment tunnel | Stateless connector with privileged API credential | Cloudflare/GitHub workflow configuration | Kubernetes deployment mutations; workflow identity must be scoped | Connector capacity | Recovery overlay at replicas 0 | Keep home as the only live connector until the Access route, independently scoped credential, and split-brain deployment tests are complete; PR #233 adds the Oracle standby gate. See homelab #204. |
| JMusicBot | Stateful external bot | R2-backed configuration/state mirror | Discord voice/music messages; Oracle egress is constrained | Active home capacity, one writer | Recovery overlay at replicas 0 | Do not run active-active Discord voice writers. Promote only after R2 freshness/config restore, Discord session fencing, duplicate-work checks, startup/health proof, and measured Oracle egress/audio behavior. PR #228 adds the Oracle standby guard; existing migration runbook applies. |
