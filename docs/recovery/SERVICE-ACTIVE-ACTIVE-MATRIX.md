...
|---|---|---|---|---|---|---|
| PantryBot | Stateful, event-processing | PostgreSQL target; SQLite retained only for rollback/migration | Twitch chat/EventSub, overlay, moderation; PostgreSQL lease/fencing per target | Active application capacity | Active application capacity | Split gateway, durable queue, workers, transactional outbox, dispatchers, overlays, public site, and private API are deployed at both sites. Remaining gates are database promotion/old-writer fencing, public failover routing, and measured cross-site failure evidence. See PantryBot #147 and homelab #191. |
...
| Authentik + PostgreSQL | Stateful identity with an active-active application tier | Oracle PostgreSQL writer with home-side recovery capacity and one fenced writer per epoch | Identity/session issuance; both sites serve application capacity, while database promotion and epoch fencing prevent stale writers | Active server/worker/LDAP capacity; no local DB writer in the current failover state | Active server/worker/LDAP capacity; current DB writer | Application capacity is live on both sites. Current authority is Oracle after the controlled failover state; return-home and repeatable promotion/fencing remain gates, along with provider/session parity, routing, and rollback. See homelab #198/#202. |
