...
| Authentik + PostgreSQL | Stateful identity with an active-active application tier | Home PostgreSQL writer with Oracle streaming standby and one fenced writer per epoch | Identity/session issuance; both sites serve application capacity, while database promotion and epoch fencing prevent stale writers | Active server/worker/LDAP capacity; current DB writer | Active server/worker/LDAP capacity; read-only DB standby | Application capacity is live on both sites. Remaining gates are provider/session parity in a restored target, promotion/fencing, routing, and rollback. See homelab #198/#202. |
