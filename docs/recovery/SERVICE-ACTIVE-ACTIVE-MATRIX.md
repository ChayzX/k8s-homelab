...
|---|---|---|---|---|---|---|
| PantryBot | Stateful, event-processing | PostgreSQL target; SQLite retained only for rollback/migration | Twitch chat/EventSub, overlay, moderation; PostgreSQL lease/fencing per target | Active application capacity and current writer | UI/application capacity plus a separately named, read-only PostgreSQL reseed candidate streaming from home; application roles remain stopped | Controlled live promotion/failback and stale-writer fencing are **not yet proven**. Remaining gates are repeated freshness capture, canonical standby/service cutover, production stale-writer rejection, provider-side external-effect semantics, public failover routing, and broader site/failure evidence. GHCR-authenticated component rollout is verified. See PantryBot #147 and homelab #191. |
