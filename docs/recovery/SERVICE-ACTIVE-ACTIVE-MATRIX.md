...
|---|---|---|---|---|---|---|
| PantryBot | Stateful, event-processing | PostgreSQL authority; one fenced writer per epoch | Twitch chat/EventSub, overlay, moderation; PostgreSQL lease/fencing per target | Split HTTP/UI, worker, overlay, gateway, dispatcher, and tunnel capacity Ready against Oracle writer; database fenced | Writable promoted database and matching active application capacity | Reference implementation. Both sites have Ready split capacity; Oracle owns all external-side-effect leases. Live synthetic queue/outbox duplicate-suppression proof is complete; provider-side EventSub/dispatcher proof, return-home, and automatic promotion remain open. See PantryBot #147 and homelab #191. |
...
| Authentik + PostgreSQL | Stateful identity | PostgreSQL PVC/physical standby; one fenced writer per epoch | Identity/session issuance; duplicate active writers are unsafe | Fenced database; stateless HTTP capacity against Oracle during current failover | Writable promoted database plus server/worker/LDAP capacity | Controlled failover promoted Oracle after fencing home and verified external auth routes. Full interactive provider/session and return-home evidence remain open. See homelab #168/#198/#202. |
| Observability (Prometheus/Loki/Grafana) | Stateful history, stateless collectors | Local PVCs plus Grafana Cloud long-lived metrics/log paths | Alert delivery and dashboards; collectors can duplicate, alert evaluation needs dedupe | Local collector/history plus bounded Cloud write path | Oracle-labeled Prometheus collector plus bounded Cloud write path | Both collectors are live with zero post-mitigation remote-write failures; the Cloud export is allowlisted and request pressure is bounded. Cloud account usage/read validation and external alert continuity remain open. See homelab #203. |
...
| JMusicBot | Stateful external bot | R2-backed configuration/state mirror | Discord voice/music messages; resource-scoped witness lease and fencing; Oracle egress is constrained | Active capacity, lease-controlled writer | Active process capacity, lease-controlled promotion | Both sites may run the process, but only the `jmusicbot` witness lease holder may open Discord or sync R2 state. Promotion remains gated on witness provisioning, R2 freshness/config restore, duplicate-work checks, startup/health proof, and measured Oracle egress/audio behavior. Existing migration runbook applies. |
