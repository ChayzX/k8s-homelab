...
| Home node loss | CI connector unavailable and home API route fails | Determine whether the other home node has capacity; connector is not proof the API server is available | Do not send workflow jobs to Oracle without an explicitly scoped Oracle target/credential. Promote only through a controlled deployment-target change. | A non-production deployment to the intended target records target/site identity and succeeds. |
| Home outage/WAN loss | GitHub Actions cannot reach the intended private API | Check Cloudflare edge, connector, origin API, and workflow configuration separately | Disable/reject jobs targeting the unavailable site; use Oracle only if its independent connector and RBAC are prepared. Never let both connectors race the same rollout. | Stale home target is rejected, Oracle target is explicit, and rollback to home is possible. |
