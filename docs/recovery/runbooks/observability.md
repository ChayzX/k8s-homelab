...
| Split-brain/duplicate alert evaluators | Repeated Discord alerts, two watcher processes, duplicate Grafana notifications | Compare both `sudo systemctl` and `systemctl --user`, pod lists, alert fingerprints, cooldown state, and watcher logs. A user-level unit can duplicate a system-level watcher even when the system service has zero restarts. Do not suppress all errors to quiet symptoms. | Keep one designated host watcher/evaluator per alert contract; disable the duplicate unit, preserve cooldown state, and reload the surviving service. Cloudflared readiness suppression applies only to exact known healthy connector messages. | One alert per incident/cooldown, real readiness/origin errors still alert, and controlled test reaches both Discord/Operations paths if configured. |
...
3. Check for duplicate system and user-level watcher units before changing
   alert thresholds:

```bash
sudo systemctl is-active k3s-watcher.service
systemctl --user is-active k3s-watcher.service
pgrep -af '[w]atcher.py'
```

   Keep the system unit as the sole evaluator. Disable a stale user unit and
   preserve the cooldown ledger; do not run two evaluators against the same
   Discord recipient.
4. If the watcher itself is producing a storm, stop only the watcher process
