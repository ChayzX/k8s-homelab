...
RoleBindings, and token Secrets **inside** the existing `pantry-bot` and
`opsbot` namespaces (see `../_bootstrap/00-namespaces.yaml`). Nothing here
...
```bash
# 0. prerequisite — pantry-bot and opsbot namespaces already exist
...
at the target namespace). Two separate kubeconfigs, one per token — the
...
ServiceAccount in its own namespace.
