...
- **Not started at all**: none remaining in this repo's tracked migration scope. `playit.service` is still `active`/`enabled` on the host — unresolved loose end, not part of this design, flagged since Minecraft's Phase-4 cleanup (section 4) hasn't reached it yet.
...
**Live status: applied and live, migrated.** `msh.service` is `inactive` (confirmed via `systemctl is-active`); the `minecraft` LoadBalancer Service is bound at `192.168.40.208:25565` and the `minecraft` pod is `1/1 Running`. `playit.service` is still `active`/`enabled` on the host — the runbook's last cleanup step (disable it only after the new Service is confirmed bound and joinable) has not yet been executed; not a blocker, just an open loose end.
