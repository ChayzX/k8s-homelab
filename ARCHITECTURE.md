...
**Logging correction:** the retired Docker-side collector and duplicate Pantry Bot containers were stopped after the Kubernetes cutover. Alloy now ships Kubernetes pod logs to Loki; no legacy collector is required.
...
| `observability` | `observability/namespace.yaml` | Loki, Prometheus, Grafana, alloy-logs, kube-state-metrics; in-cluster alerts come from k3s-watcher and external reachability is observed by the GCP monitor |
...
|---|---|---|
| `loki.observability` | 3100 (http), 9095 (grpc) | Log storage; Grafana datasource + Alloy push target |
...
| `alloy-logs.observability` | 12345 | Alloy logs metrics, scraped by Prometheus |
...
`prometheus`, `grafana`, `alloy-logs` (DaemonSet), and `kube-state-metrics` — are
healthy. Uptime Kuma was decommissioned; k3s-watcher handles in-cluster
alerting and the independent GCP monitor handle external reachability. The old Docker collector was retired 2026-09-19.
...
| `alloy-logs` (**DaemonSet**, not Deployment — log shipper needs one pod per node) | `grafana/alloy:v1.8.3` | 12345 | 50m/200m CPU, 64Mi/256Mi mem | `10001:10001` (non-root) | `alloy-logs-sa`, **yes** (`kubernetes_sd_configs`, role: pod) | none (emptyDir for data) |
...
Alloy tails pod logs via the Kubernetes API instead of hostPath mounts; the retired collector's hostPath mounts are no longer needed.
...
- **Every workload gets its own dedicated ServiceAccount**, named `<app>-sa`, namespaced with the app. `automountServiceAccountToken: false` is set explicitly wherever the app has no legitimate reason to call the Kubernetes API — which is most of them. Workloads that DO need API access get a narrow, purpose-built Role: `prometheus-sa` (service discovery + kubelet/cAdvisor scraping), `alloy-logs` (pod discovery for log labeling), `kube-state-metrics-sa` (broad read-only listing — that's its whole job), `opsbot-sa` (pods/deployments in an explicit namespace allowlist, see its own section), and `ci-deploy` (deployments/replicasets only, one per deployed namespace, used by GitHub Actions — see `ci-deploy/README.md`). `keel`'s SA is gone along with the rest of Keel (removed 2026-08-13).
...
**How the severity filter actually works, mechanically**: Alloy's `processing stages` (`observability/alloy-logs-home.yaml`) extract a `level` value per line via regex/JSON and promote it to a real Loki **label** via a `labels` stage — not just a parsed field. That's what makes `{namespace=~"$namespace", level=~"$level"}` work as a stream selector Loki resolves before reading log bodies, rather than a slow client-side text scan. If a given app's parsing stage ever breaks, its logs don't vanish — they fall into "unclassified" (a dedicated stat panel exists specifically to catch that).
