...
Same gap, smaller version: point `FUNCTIONAL_HEALTH_URLS` and `WATCH_NAMESPACES` at `opsbot` and `observability` so a repeat of the 25-day-silent `loki-query` failure or the 10-hour-silent `opsbot` ImagePullBackOff actually pages you.
...
1. **Quick wins, no architecture risk:** DHCP reservation for `minecraftmachine`'s WiFi MAC (not done — router access, only you can); add `observability` and functional-health coverage for `opsbot` to `k3s-watcher` (not done); investigate/fix `loki-query` (not done).
2. **Multi-arch CI for `jmusicbot` and `opsbot`** — hard prerequisite for Tier-1 status, mirrors the pantry-bot/opsbot-original migration already in `ARCHITECTURE.md`. Not started.
3. **Extend the Litestream/R2 + emptyDir pattern** to `jmusicbot` and `opsbot`'s state, matching pantry-bot exactly. Not started.
