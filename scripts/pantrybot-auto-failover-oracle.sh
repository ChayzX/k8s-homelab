...
Required environment for --confirm:
  HOME_PRIMARY_PROBE_URL        private home-primary readiness URL
...
: "${HOME_PRIMARY_PROBE_URL:?HOME_PRIMARY_PROBE_URL is required}"
...
if [[ "$mode" == "--dry-run" ]]; then
  printf '%s\n' \
    probe_source=private-home-primary \
...
for attempt in $(seq 1 "$HOME_FAILURE_THRESHOLD"); do
  if curl --fail --silent --show-error --max-time "${HOME_PROBE_TIMEOUT_SECONDS:-5}" \
      "$HOME_PRIMARY_PROBE_URL" >/dev/null; then
